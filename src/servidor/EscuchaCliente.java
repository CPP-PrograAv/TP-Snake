		...
		while (dt < 2 * 60 * 60 * 1000) {
			try {
				...
				case Parametro.ACTUALIZAR_LOBBY:
					...
					ArrayList<SalaEspera> salas = server.getvSalaEspera();
					salida.flush();
				...
				case Parametro.EMPEZAR_JUEGO:
					...
					for (EscuchaCliente ec : clientesEnSala) {
						// juego.addJugador(new
						// Jugador(ec.persona.getNick(),0));
						ec.salida.flush();
						// ec.salida.writeObject(new
						// Message(Parametro.EMPEZAR_JUEGO,juego);
					}
					break;
